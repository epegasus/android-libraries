...
import com.sohaib.collageview.R
import com.sohaib.collageview.utils.ImageParams
import com.sohaib.collageview.utils.TaskRunner
import java.util.*
...
abstract class AbstractCollageView(
    context: Context, attrs: AttributeSet?, private val imageCount: Int, var layoutWidth: Int = 0,
    var layoutHeight: Int = 0, var isBorderEnabled: Boolean = false, protected var imageUris: Array<Uri?>? = null
    ...
    protected fun addImagesToViews() {
        val uris = imageUris ?: return
        for (i in 0 until imageCount) {
            setImageAt(i, uris.getOrNull(i))
        }
    }
    ...
    override fun setImageAt(index: Int, resId: Int) {
        when {
            index >= imageViews.size -> Log.d(TAG, "setImageAt(resId): invalid index")
            else -> loadImageInView(index, resId)
        }
    }

    fun setImageAt(index: Int, drawable: Drawable?) {
        when {
            index >= imageViews.size -> Log.d(TAG, "setImageAt(drawable): invalid index")
            drawable == null -> loadDefaultImageInView(index)
            else -> loadImageInView(index, drawable)
        }
    }

    fun setImageAt(index: Int, bitmap: Bitmap?) {
        when {
            index >= imageViews.size -> Log.d(TAG, "setImageAt(bitmap): invalid index")
            bitmap == null -> loadDefaultImageInView(index)
            else -> loadImageInView(index, bitmap)
        }
    }
    ...
    private fun loadImageInView(index: Int, resId: Int) {
        imageViews[index].apply {
            scaleType = ImageView.ScaleType.CENTER
            ...
            setTag(R.id.image_source, "res:$resId")
        }
    }

    private fun loadImageInView(index: Int, drawable: Drawable) {
        imageViews[index].apply {
            scaleType = ImageView.ScaleType.CENTER
            ...
            setTag(R.id.image_source, "drawable")
        }
    }

    private fun loadImageInView(index: Int, bitmap: Bitmap) {
        imageViews[index].apply {
            scaleType = ImageView.ScaleType.CENTER
            ...
            setTag(R.id.image_source, "bitmap")
        }
    }
